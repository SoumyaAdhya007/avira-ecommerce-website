...
async function signUpFun() {
	...
	if(password!=confirmPassword){
		...
		try {
			let signup_res= await fetch(`${baseURL}/admins/register`,{
				method:"POST",
				headers:{
...
async function signin(){
	try {
	  ...
	  console.log(obj)
	  let login_request= await fetch(`${baseURL}/admins/login`,{
		method:"POST",
		headers:{
		  "Content-Type":"application/json"
		},
	  ...
	  if(login_request.ok){
		...
		localStorage.setItem("admin-token",token.token);
		localStorage.setItem("admin-id",token.adminId);
